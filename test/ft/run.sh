...
#
# AH_FACTORIO    the game binary, if it is not where Steam puts it here
# AH_FT_DATA     the throwaway data directory the run happens in
#
...
factorio="${AH_FACTORIO:-/home/sparr/Games/Steam/steamapps/common/Factorio/bin/x64/factorio}"
...
data="${AH_FT_DATA:-$HOME/.cache/ah-factorio-test}"
...
# ah-tests carries no prototypes; it is the gate that keeps the fixtures from registering
...
mkdir -p "$data/mods"
ln -sfn "$root/test/ft/ah-tests" "$data/mods/ah-tests"
